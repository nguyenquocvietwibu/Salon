...
const model = {
    ...
    them: async (ten, dia_chi, hinh_hoc_dia_ly) => {
        const sql = `
            INSERT INTO chi_nhanh (ten, dia_chi, hinh_hoc_dia_ly)
            VALUES ($1, $2, ST_GeomFromGeoJSON($3))
            RETURNING ma, ten, dia_chi, ST_AsGeoJSON(hinh_hoc_dia_ly)::json AS hinh_hoc_dia_ly;
        `;
        const ketQua = await query(sql, [ten, dia_chi, hinh_hoc_dia_ly]);
        return ketQua.rows[0];
    },

    // 3. Cập nhật thông tin chi nhánh
    capNhat: async (ma, ten, dia_chi, hinh_hoc_dia_ly) => {
        const sql = `
            UPDATE chi_nhanh 
            SET ten = $1, dia_chi = $2, hinh_hoc_dia_ly = ST_GeomFromGeoJSON($3)
            WHERE ma = $4
            RETURNING ma, ten, dia_chi, ST_AsGeoJSON(hinh_hoc_dia_ly)::json AS hinh_hoc_dia_ly;
        `;
        const ketQua = await query(sql, [ten, dia_chi, hinh_hoc_dia_ly, ma]);
        return ketQua.rows[0];
    },
