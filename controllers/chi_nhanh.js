...
const controller = {
    ...
    addBranch: async (req, res) => {
        try {
            const { ten, dia_chi, hinh_hoc_dia_ly, dang_kich_hoat } = req.body;

            // Bắt buộc phải điền đủ thông tin cơ bản
            if (!ten || !dia_chi || !hinh_hoc_dia_ly) {
                return res.status(400).json({
                    success: false,
                    message: "Vui lòng nhập đầy đủ tên, địa chỉ và tọa độ chi nhánh!"
                });
            }

            const geoJsonString = JSON.stringify(hinh_hoc_dia_ly);

            // Mặc định là true nếu client không truyền dang_kich_hoat
            const isActivate = typeof dang_kich_hoat === 'boolean' ? dang_kich_hoat : true;

            const data = await chiNhanhModel.them(ten, dia_chi, geoJsonString, isActivate);
    ...
    // 🌟 API Cập nhật toàn bộ thông tin chi nhánh
    updateBranch: async (req, res) => {
        try {
            ...
            const data = await chiNhanhModel.capNhat(ma, ten, dia_chi, geoJsonString, dang_kich_hoat);

            if (!data) {
                return res.status(404).json({
                    success: false,
                    message: "Không tìm thấy chi nhánh cần cập nhật!"
                });
            }
    ...
    getById: async (req, res) => {
        try {
            ...
            if (!ma || isNaN(ma)) {
                return res.status(400).json({
                    success: false,
                    message: "Mã chi nhánh (ma) không hợp lệ!"
                });
            }

            const data = await chiNhanhModel.layTheoMa(ma);
